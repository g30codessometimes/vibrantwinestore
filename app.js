...
async function init() {
  if (CONFIG.CLIENT_NAME) {
    ...
    $("clientLabel").style.display = "inline";
  }
  if (CONFIG.MOQ_NOTE) {
...
function parsePortfolio(html) {
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    const photos = Array.from(section.querySelectorAll(".bandeau img")).map(img => {
      ...
      return src ? new URL(src, CONFIG.PORTFOLIO_URL).href : "";
    }).filter(Boolean);
...
function producerSection(producer, visibleWines) {
  const photosHtml = producer.photos.length
    ? `<div class="producer-photos">${producer.photos.slice(0, 3).map(src => `<img src="${escapeHtml(src)}" alt="" loading="lazy">`).join("")}</div>`
    : "";
